...
function Vector2(_x=0, _y=0) constructor
{
	...
	/// @desc	Returns the angle (in degrees) of the vector.
	static getAngleDegrees = function()
	{
		return point_direction(0, 0, x, y);
	}
	
	/// @func	getAngleRadians();
	/// @desc	Returns the angle (in radians) of the vector.
	static getAngleRadians = function()
	{
		return degtorad(getAngleDegrees());
	}
	...
	/// @func	rotateRadians({real} angle);
	/// @desc	Rotates the vector by the given angle (in radians).
	static rotateRadians = function(_angle)
	{
		rotateDegrees(radtodeg(_angle));
	}
	
	/// @func	dot({real} x, {real} y);
	static dot = function(_x, _y)
	{
		return x * _x + y * _y;
	}
	
	/// @func	dotVector({Vector2} other);
	static dotVector = function(_other)
	{
		return dot(_other.x, _other.y);
	}
